...
import com.contrarywind.data.WheelDividerTypeEnum
...
class WheelOptions<T>(
    ...
    private val wv_option1: WheelView
    private val wv_option2: WheelView
    private val wv_option3: WheelView
    ...
    fun setPicker(
        ...
        // 选项1
        wv_option1.setAdapter(ArrayWheelAdapter<Any>(mOptions1Items)) // 设置显示数据
        wv_option1.currentItem = 0 // 初始化时显示的数据
        // 选项2
        if (mOptions2Items != null) {
            wv_option2.setAdapter(ArrayWheelAdapter<Any>(mOptions2Items!![0])) // 设置显示数据
        }
        wv_option2.currentItem = wv_option2.currentItem // 初始化时显示的数据
        // 选项3
        if (mOptions3Items != null) {
            wv_option3.setAdapter(ArrayWheelAdapter<Any>(mOptions3Items!![0]?.get(0))) // 设置显示数据
        }
        ...
        wheelListener_option1 = object : OnItemSelectedListener {
            override fun onItemSelected(index: Int) {
                ...
                if (mOptions2Items == null) { //只有1级联动数据
                    ...
                    wv_option2.setAdapter(ArrayWheelAdapter<Any?>(mOptions2Items!![index]))
        ...
        wheelListener_option2 = object : OnItemSelectedListener {
            override fun onItemSelected(index: Int) {
                ...
                if (mOptions3Items != null) {
                    ...
                    _index =
                        if (_index >= mOptions2Items!![opt1Select]?.size!! - 1) mOptions2Items!![opt1Select]?.size!! - 1 else _index
                    var opt3 = 0
                    if (!isRestoreItem) {
                        // wv_option3.getCurrentItem() 上一个opt3的选中位置
                        //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                        opt3 =
                            if (wv_option3.currentItem >= mOptions3Items!![opt1Select]!![_index]?.size!! - 1) mOptions3Items!![opt1Select]!![_index]?.size!! - 1 else wv_option3.currentItem
                    }
                    wv_option3.setAdapter(ArrayWheelAdapter<Any?>(mOptions3Items!![wv_option1.currentItem]!![_index]))
    ...
    fun setNPicker(options1Items: List<T?>?, options2Items: List<T?>?, options3Items: List<T?>?) {
        ...
        // 选项1
        wv_option1.setAdapter(ArrayWheelAdapter(options1Items)) // 设置显示数据
        wv_option1.currentItem = 0 // 初始化时显示的数据
        // 选项2
        if (options2Items != null) {
            wv_option2.setAdapter(ArrayWheelAdapter(options2Items)) // 设置显示数据
        }
        wv_option2.currentItem = wv_option2.currentItem // 初始化时显示的数据
        // 选项3
        if (options3Items != null) {
            wv_option3.setAdapter(ArrayWheelAdapter(options3Items)) // 设置显示数据
        }
        wv_option3.currentItem = wv_option3.currentItem
        wv_option1.setIsOptions(true)
        wv_option2.setIsOptions(true)
    ...
    val currentItems: IntArray
        get() {
            ...
            if (mOptions3Items != null && mOptions3Items!!.isNotEmpty()) { //非空判断
                currentItems[2] =
                    if (wv_option3.currentItem > mOptions3Items!![currentItems[0]]?.get(currentItems[1])?.size!! - 1) 0 else wv_option3.currentItem
            } else {
    ...
    private fun itemSelected(opt1Select: Int, opt2Select: Int, opt3Select: Int) {
        ...
        if (mOptions2Items != null) {
            wv_option2.setAdapter(ArrayWheelAdapter<Any?>(mOptions2Items!![opt1Select]))
            wv_option2.currentItem = opt2Select
        }
        if (mOptions3Items != null) {
            wv_option3.setAdapter(ArrayWheelAdapter<Any?>(mOptions3Items!![opt1Select]!![opt2Select]))
     ...
     * @param dividerType
     */
    fun setDividerType(dividerType: WheelDividerTypeEnum?) {
    ...
    init {
        wv_option1 = view.findViewById<View>(R.id.options1) as WheelView // 初始化时显示的数据
