...
object ChinaDate {
    ...
    fun calElement(y: Int, m: Int, d: Int): LongArray {
        ...
        while (i < 13 && offset > 0) {
            ...
            // 闰月
            if (leap > 0 && i == leap + 1 && val6 == 0L) {
                --i
                val6 = 1L
                temp = leapDays(nongDate[0].toInt())
            } else {
                temp = monthDays(nongDate[0].toInt(), i)
            }
            // 解除闰月
            if (val6 == 1L && i == leap + 1) nongDate[6] = 0
            offset -= temp.toLong()
            if (val6 == 0L) nongDate[4]++
            i++
        }
    ...
    val currentLunarDate: String
        get() {
            val today = Calendar.getInstance(Locale.SIMPLIFIED_CHINESE)
            val year = today[Calendar.YEAR]
            val month = today[Calendar.MONTH] + 1
            val date = today[Calendar.DATE]
            val l = calElement(year, month, date)
    ...
    fun oneDay(year: Int, month: Int, day: Int): String {
        ...
        val l = calElement(year, month, day)
        var sToday: StringBuffer? = StringBuffer()
        return try {
