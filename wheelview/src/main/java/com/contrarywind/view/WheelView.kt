...
import com.contrarywind.data.WheelActionEnum
...
class WheelView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    //偏移量
    private var CENTER_CONTENT_OFFSET = 0f
    ...
    private fun reMeasure() { //重新测量
        ...
        if (initPosition == -1) {
            initPosition = if (isLoop) {
                (adapter!!.itemsCount?.plus(1))?.div(2)!!
            } else {
                0
            }
        }
    ...
    fun smoothScroll(action: WheelActionEnum) { //平滑滚动的实现
        cancelFuture()
        if (action == WheelActionEnum.FLING || action == WheelActionEnum.DAGGLE) {
    ...
    fun setItemsVisibleCount(visibleCount: Int) {
        var val1 = visibleCount
        if (val1 % 2 == 0) {
            val1 += 1
        }
        itemsVisible = val1 + 2 //第一条和最后一条
    }
    ...
    var currentItem: Int
        get() {
            ...
            return if (isLoop && (selectedItem < 0 || selectedItem >= adapter?.itemsCount!!)) {
                max(
                    0,
                    abs(abs(selectedItem) - adapter?.itemsCount!!).coerceAtMost(adapter?.itemsCount!! - 1)
                )
            } else max(0, min(selectedItem, adapter?.itemsCount!! - 1))
        }
        set(currentItem) {
            //不添加这句,当这个wheelView不可见时,默认都是0,会导致获取到的时间错误
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        initPosition = min(max(0, initPosition), adapter?.itemsCount!! - 1)
        ...
        //绘制中间两条横线
        if (dividerType == WheelDividerTypeEnum.WRAP) { //横线长度仅包裹内容
            var startX: Float
            val endX: Float
        ...
        } else if (dividerType == WheelDividerTypeEnum.CIRCLE) {
            //分割线为圆圈形状
    ...
    private fun getLoopMappingIndex(index: Int): Int {
        var val1 = index
        if (val1 < 0) {
            val1 += adapter?.itemsCount!!
            getLoopMappingIndex(val1)
        } else if (val1 > adapter?.itemsCount!! - 1) {
            val1 -= adapter?.itemsCount!!
            getLoopMappingIndex(val1)
        }
        return val1
    ...
    private fun getContentText(item: Any?): String {
        if (item == null) {
            ...
            return item.pickerViewText.toString()
        } else if (item is Int) {
            //如果为整形则最少保留两位数.
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> if (!eventConsumed) { //未消费掉事件
                ...
                if (System.currentTimeMillis() - startTime > 120) {
                    // 处理拖拽事件
                    smoothScroll(WheelActionEnum.DAGGLE)
                } else {
                    // 处理条目点击事件
                    smoothScroll(WheelActionEnum.CLICK)
                }
            }
    ...
    init {
        ...
        if (attrs != null) {
            ...
            textColorCenter =
                a.getColor(R.styleable.pickerview_wheelview_textColorCenter, -0xd5d5d6)
